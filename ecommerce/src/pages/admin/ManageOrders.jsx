...
import { PrinterIcon, SearchIcon, Trash2, ChevronLeft, ChevronRight } from "lucide-react"; // Added Trash2 and ChevronLeft/Right
      ...
      <div className="admin-card">
        <div className="filters-bar">
          <div className="search-box">
            ...
            />
            <SearchIcon className="w-4 h-4 text-gray-500" /> {/* Added text color */}
          </div>

          <div className="filter-options">
            <div className="filter-group">
              <label className="text-gray-700 dark:text-gray-300">Status:</label> {/* Added text color */}
              <select
                value={statusFilter}
            ...
            <div className="filter-group">
              <label className="text-gray-700 dark:text-gray-300">Payment:</label> {/* Added text color */}
              <select
                value={paymentFilter}
              ...
              >
                ...
                <option value="pending">Pending</option> {/* Changed order to match backend */}
                <option value="completed">Completed</option> {/* Changed from paid to completed */}
                <option value="failed">Failed</option> {/* Added failed status */}
                <option value="refunded">Refunded</option>
              </select>
            </div>

            <div className="filter-group">
              <label className="text-gray-700 dark:text-gray-300">Sort By:</label> {/* Added text color */}
              <select
                value={sortBy}
          ...
          <table className="orders-table">
            ...
            <tbody>
              {currentOrders.length === 0 ? (
                <tr>
                  <td colSpan="6" className="no-results text-center py-4"> {/* Adjusted colspan and added styling */}
                ...
                currentOrders.map((order) => (
                  <tr key={order.id}>
                    <td>
                      ...
                      <Link to={`/admin/orders/${order.id}`} className="order-number hover:underline text-indigo-600 dark:text-indigo-400">
                         {order.order_number}
                      </Link>
                    </td>
                    <td>
                      <div className="customer-name">{order.user?.name || 'N/A'}</div> {/* Use optional chaining */}
                    </td>
                  ...
                  <td>
                      <div className="action-buttons flex items-center space-x-2"> {/* Added flex and spacing */}
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="pagination flex justify-center items-center mt-4 space-x-2"> {/* Added flex and spacing */}
            <button
